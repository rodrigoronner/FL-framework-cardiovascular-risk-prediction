"""
experiments/run_dp_sensitivity_proportional.py
========================================
Non-fuzzy ablation baseline for run_dp_sensitivity_fuzzy.py, requested by
peer review: recommends a per-client batch size scaled linearly by
dataset size alone (dpfedadam.proportional_fairness.
ProportionalFairnessController; no epsilon feedback, no fuzzy inference)
at the adopted DP configuration (sigma=2.5, local_epochs=1 - see
run_privacy_budget_test.py). Computes each client's baseline epsilon at
a fixed L=32, derives a per-client batch size proportional to n_train,
...
comparable to run_privacy_budget_test.py's fixed-batch numbers and
run_dp_sensitivity_fuzzy.py's fuzzy-rebalanced numbers.

Usage
-----
    python -m experiments.run_dp_sensitivity_proportional --data_dir ./data --rounds 100 \
...
from dpfedadam.proportional_fairness import FairnessRebalanceReport, ProportionalFairnessController
...
DP_SCENARIOS: Dict[str, Dict] = {
    "Adopted DP config (σ=2.5, epochs=1, proportional batch)": {"noise_multiplier": 2.5, "max_grad_norm": 1.0},
}

LINE_STYLES = {
    "Adopted DP config (σ=2.5, epochs=1, proportional batch)": ("-.", "tab:blue"),
}


def _proportional_rebalance(
...
def run(
    ...
    proportional_reports: Dict[str, FairnessRebalanceReport] = {}

    for name, dp_cfg in DP_SCENARIOS.items():
        print(f"\n{'='*60}\n  Running: {name}  (proportional-rebalanced batch sizes)\n{'='*60}")

        report = _proportional_rebalance(
            n_train_per_client, dp_cfg["noise_multiplier"], num_rounds,
            local_epochs, client_labels,
        )
        proportional_reports[name] = report
        print(report)
    ...
    calibrated_csv_path = os.path.join(out_dir, "calibrated_dp_sensitivity_proportional_metrics.csv")
    ...
    per_client_csv_path = os.path.join(out_dir, "calibrated_dp_sensitivity_proportional_per_client.csv")
    ...
    # -------------------------------------------------------------------
    proportional_rows = []
    for name, report in proportional_reports.items():
    ...
    proportional_csv_path = os.path.join(out_dir, "proportional_epsilon_rebalancing.csv")
    pd.DataFrame(proportional_rows).to_csv(proportional_csv_path, index=False)
    print(f"Proportional epsilon rebalancing report saved to: {proportional_csv_path}")
    ...
    metrics_csv_path = os.path.join(out_dir, "dp_sensitivity_proportional_metrics.csv")
    ...
    fig.suptitle(
        f"DP-FedAdam + Proportional (Non-Fuzzy) Rebalancing - DP Sensitivity, {num_rounds} rounds, {num_clients} clients",
        fontsize=15,
    )
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plot_path = os.path.join(out_dir, "dp_sensitivity_proportional_plot.png")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="DP-FedAdam - DP sensitivity with proportional per-client batch-size rebalancing"
    )
